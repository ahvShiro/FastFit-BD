...
SELECT telefones.id, telefones.numero, tipos_telefone.tipo AS tipo_telefone, telefones.criado_em, telefones.atualizado_em
FROM "telefones"
JOIN "tipos_telefone" ON telefones.id_tipo_telefone = tipos_telefone.id;

-- Relatório de Endereços
SELECT id, estado, cidade, bairro, rua, numero, complemento, cep, criado_em, atualizado_em
FROM "enderecos";
...
SELECT dados_corporais.id, objetivos.nome_objetivo, dados_corporais.idade, dados_corporais.sexo, dados_corporais.altura, dados_corporais.peso, dados_corporais.porcentagem_gordura, niveis_atividade.nome AS nivel_atividade, dados_corporais.criado_em, dados_corporais.atualizado_em
...
SELECT clientes.id, clientes.nome, clientes.sobrenome, clientes.email, enderecos.cidade, enderecos.estado, telefones.numero AS telefone, clientes.criado_em, clientes.atualizado_em
FROM "clientes"
...
SELECT restaurantes.id, restaurantes.nome_fantasia, restaurantes.cnpj, enderecos.cidade, enderecos.estado, telefones.numero AS telefone, restaurantes.pedido_minimo, restaurantes.taxa_entrega, restaurantes.criado_em, restaurantes.atualizado_em
FROM "restaurantes"
...
-- Relatório de Pratos
SELECT pratos.id, pratos.nome, pratos.descricao, pratos.preco, restaurantes.nome_fantasia AS restaurante, macros.carboidratos, macros.proteinas, macros.gorduras
FROM "pratos"
...
SELECT ingredientes_pratos.id, ingredientes.ingrediente, pratos.nome AS prato, ingredientes_pratos.quantidade, ingredientes_pratos.unidade_de_medida, ingredientes_pratos.criado_em
...
SELECT avaliacoes_restaurantes.id, restaurantes.nome_fantasia AS restaurante, avaliacoes_restaurantes.nota, avaliacoes_restaurantes.comentario, clientes.nome AS cliente, avaliacoes_restaurantes.criado_em, avaliacoes_restaurantes.atualizado_em
...
SELECT avaliacoes_pratos.id, pratos.nome AS prato, avaliacoes_pratos.nota, avaliacoes_pratos.comentario, clientes.nome AS cliente, avaliacoes_pratos.criado_em, avaliacoes_pratos.atualizado_em
...
SELECT restricoes_alimentares.id, clientes.nome AS cliente, ingredientes.ingrediente, restricoes_alimentares.criado_em, restricoes_alimentares.atualizado_em
...
-- Relatório de Cupons
SELECT id, codigo, descricao, desconto, data_inicio, data_fim, criado_em, atualizado_em
FROM "cupons";

-- Relatório de Preferências
SELECT preferencias.id, clientes.nome AS cliente, pratos.nome AS prato, preferencias.filtro_usado, preferencias.criado_em, preferencias.atualizado_em
FROM "preferencias"
...
SELECT id, tipo_refeicao, calorias, proteinas, carboidratos, fibras, gorduras_trans, sodio, gorduras_saturadas
FROM "necessidades_nutricionais";

-- Relatório de Necessidades de Clientes
SELECT necessidades_clientes.id, clientes.nome AS cliente, necessidades_nutricionais.tipo_refeicao, necessidades_clientes.criado_em, necessidades_clientes.atualizado_em
FROM "necessidades_clientes"
JOIN "clientes" ON necessidades_clientes.id_cliente = clientes.id
JOIN "necessidades_nutricionais" ON necessidades_clientes.id_necessidades_nutricionais = necessidades_nutricionais.id;

-- Relatório de Pedidos Pratos
SELECT pedidos_pratos.id, pedidos.id AS pedido, pratos.nome AS prato, pedidos_pratos.quantidade, pedidos_pratos.criado_em, pedidos_pratos.atualizado_em
FROM "pedidos_pratos"
...
GROUP BY
    pratos.nome
ORDER BY
    AVG(avaliacoes_pratos.nota) DESC
LIMIT 5;
